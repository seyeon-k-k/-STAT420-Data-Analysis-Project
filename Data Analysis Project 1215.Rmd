...
The objective of this report is to predict wine quality based on various chemical features such as alcohol content, pH, and volatile acidity. Understanding and predicting wine quality is not only crucial for consumers, who seek consistent and high-quality products, but also for winemakers, who can optimize production processes to enhance product quality. 

Traditionally, wine quality is typically assessed through sensory evaluation by wine experts, who rate wines based on attributes like taste, aroma, and appearance. However, sensory evaluations can be subjective, time-consuming, and costly. Therefore, developing predictive models based on measurable chemical properties would provide a more efficient and scalable solution.

Given the complexity and variability of wine production, predicting quality is a challenging task. In this study, we aim to use data from two types of wines - white and red - to predict wine quality using Machine Learning Models. Multiple regression models are explored to identify the most accurate and reliable model for predicting wine quality. The analysis includes testing several linear regression techniques, evaluating and comparing model performance in order to choose the best approach.
...
The linear regression model, which assumes linear relationship between predictors and the response variable, is our baseline model. However, given the possibility of nonlinear relationships in the data, we also build interaction and quadratic regression models. Here we try two-way interaction model and three-way interaction model and two-way interaction model with quadratic terms as well using the same predictors. 
...
```
##### 2.6.3 Refining the Model: (Further detail and code in 5.1 Appendix A)

- Stepwise Selection: We used both forward and backward stepwise selection to identify a subset of predictors that minimize the AIC. The final model excluded `chlorides`, with minimal improvement in the AIC (from 6861.1 to 6860.3).
...
```
Here, we will choose Quadractic Model and Logged Response Additive Model as they have high Adjusted R2 with comparably low CV-RMSE (Logged Response Additive Model has significantly low CV-RMSE)
...
plot(fitted(response_log_model_fix), resid(response_log_model_fix), col = "grey", pch = 20,
     xlab = "Fitted", ylab = "Residuals", main = "Fitted vs Residuals Plot")
abline(h = 0, col = "darkorange", lwd = 2)

```
Here we can see the issue with the structure of our data from Fitted vs Residuals Plot of two models. Since our response variable "quality" is discrete, we are facing problems with the residuals because the MLR model assumes continuous data and seeing violations of homoscedasticity and linearity assumption.
...
bptest(response_log_model_fix)
ad.test(resid(response_log_model_fix))
...
We categorize it into groups to simplify the classification process and make it more interpretable. Now, we try to fit the Logistic Regression Model. The LRM will predict the probability of each category and allow us to quantify the effect if each predictor on these probabilities.
